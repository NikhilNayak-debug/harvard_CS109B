...
def plot_df(df):
    ...
    sns.countplot(data=df2, x='gender', ax=axes[1], palette='coolwarm')
    axes[1].set_title('Gender Representation')

    # Plot ethnicity representation
    sns.countplot(data=df2, x='ethnicity', ax=axes[2], palette='viridis')
...
# Generator function
def data_generator(df, base_img_path, img_size=(48,48), batch_size=32):
    while True:
...
def data_generator2(df, image_data, batch_size=32):
    while True:
        # Shuffle the dataframe
        df = df.sample(frac=1).reset_index(drop=True)
